...
class HomepageIndexView(APIView):
    def get(self, request):
        try:
            ...
            if search_query:
                query_set = Character.objects.filter(
                    Q(name__icontains=search_query) | Q(profile__icontains=search_query)    # i表示忽略大小写
                )
            else:
                query_set = Character.objects.all()
            characters_raw = query_set.order_by('-id')[items_count: items_count + 20]
            characters = []
